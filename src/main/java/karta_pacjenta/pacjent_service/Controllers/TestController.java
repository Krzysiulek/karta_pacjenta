...
public class TestController {
    ...
    public void addTestUsers() {
        MyServiceUser us1 = myAppUsersRepository.save(new MyServiceUser(1L, "admin", passwordEncoder.encode("admin"), "admin@email.ru",
        ...
        MyServiceUser us2 = myAppUsersRepository.save(new MyServiceUser(2L, "patient", passwordEncoder.encode("patient"), "admin@email.ru",
                new HashSet<>(Arrays.asList(UserRoles.PATIENT.getRole())),
                "Kris", "Patient", "Kutno", 12312414, 124124424));

        MyServiceUser us3 = myAppUsersRepository.save(new MyServiceUser(3L, "patient2", passwordEncoder.encode("patient2"), "admin@email.ru",
                new HashSet<>(Arrays.asList(UserRoles.PATIENT.getRole())),
                "Blaise", "Patient", "Kutno", 12312414, 124124424));

        MyServiceUser us4 = myAppUsersRepository.save(new MyServiceUser(4L, "doctor", passwordEncoder.encode("doctor"), "admin@email.ru",
                new HashSet<>(Arrays.asList(UserRoles.DOCTOR.getRole())),
                "Blaise", "Doctor", "Kutno", 12312414, 124124424));

        patientsRepository
                .save(new Patient(5L, us1.getUserId()));

        patientsRepository
                .save(new Patient(6L, us2.getUserId()));

        patientsRepository
                .save(new Patient(7L, us3.getUserId()));



        diseasesRepository.save(new Diseases(8, "Ospa", "Kropelkowe", "blablabla to jest jakiś opis"));
